    ...
    <main>    
        <div class="container">
            <p class="mt-5" v-if="planDetail.mepr_product_period=='years'">              
                Terms: ${{planDetail.mepr_product_price}} per calendar year for premium access to our community. Membership includes 
                weekly advertising, access to CareCalendar, Last Minute Care Service, Verified Care Provider 
                Service and discounts on all background checks. Membership renews automatically. To avoid 
                charges for the next year, cancel before the renewal date.

            </p>  
            <p class="mt-5" v-else-if="planDetail.mepr_product_period=='months'">
                Terms: ${{planDetail.mepr_product_price}}/month for premium access to our community. Membership includes weekly advertising,
                access to CareCalendar, Last Minute Care Service and discounts on all background checks. 
                Cancel anytime.
            </p> 
            <p class="mt-5" v-else-if="planDetail.mepr_product_period=='lifetime'">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group  mt-3">
                        <label for="uname">State/Province:<sup class="text-danger">*</sup></label>
                        <input type="stateProvince" class="form-control" id="stateProvince" placeholder="" v-model="stateProvince">
    ...
    <script>        
        ...
        data() {
            return { 
                ...
                pageTitle : 'Seattle Nanny Agencies - Looking for Nannies? Sign Up!',
				pageMetaDescription : 'Seattle nanny agencies! Looking for nannies? We have thousands! Sign up here. Once you become a member, you will receive access to a special page....',
				pageMetaKeywords : ''
            };
        },
        methods: {
            ...
            read() {
                axios.get(`/api/buy-agencies-plan/${this.planID}`).then(({ data }) => {
            ...
            stripeDetail() {
                axios.get(`/api/payment-gateway`, {
                    ...
                    //console.log('test', response.data.patmentGateway.StripePublishablekey);
                }).catch(error => {
                });  
            },
        ...
        mounted() {
            ...
            axios.get(`/api/groups`).then((response) => {
                this.UserGroup = response.data.allGroups[2].id;
            }).catch((err) => console.error(err));
        },
        created() {
			this.stripeDetail();
